...
void printUnsignedCharArrayToHex(unsigned char in[], int size){
    for (int i = 0;i < size;i++){
        printf("%X ", in[i]);
    }
}

void binaryConverter(unsigned char* c, unsigned char* block) {
    for (int i=0; i<16; ++i) {
        int tmp = 0;
        for (int j=0; j<8; ++j) {
            int idx = i*8+j;
...
int main(){
    FILE *kp, *fp, *wp; // input file pointer, output(writer) file pointer
    int KeySize = 128; // key Size
    int feof_flag = 0; // detect end of file flag
    unsigned char key_buffer[128]; // user input Main Key, AES主Key
    ...
    fread((void*)key_buffer, sizeof(key_buffer), 1, kp);
    binaryConverter(key_buffer, Key);
    /* Key Expansion function*/
    KeyExpansion(); 
    fclose(kp);
    ...
    while(feof_flag == 1){
        ...
        for (int c = 0;c < 128;c++){
            ...
            if (feof(fp)){
                for (int padding = c;padding < 128;padding++){
                    plaintext[padding] = '0';
                }
                feof_flag = 0;
                break;
        ...
        printUnsignedCharArrayToHex(in, 16);    
        printf("\n");
        // print Cipher
        printf("Block %d - Cipher: ", blockNum); 
        printUnsignedCharArrayToHex(out, 16);
        printf("\n");
        blockNum++;
